...
num_subfoots = 4            # Number of subfootprints to be placed
...
def pyomo_create_model(options=None, model_options=None):
    ...
    # A Pyomo Set representing the chip corners
    M.C = pyomo.RangeSet(0, len(chips_to_cover)-1, ordered=True)
    ...
    def c3a_rule(M, f, c):
        return (M.Fkllx[f] - (1 - M.X1[f,c])*chip_x_len <= chips_to_cover[c][0])
    M.c3a = pyomo.Constraint(M.F, M.C, rule=c3a_rule)

    def c3b_rule(M, f, c):
        return (M.Fkurx[f] + (1 - M.X1[f,c])*chip_x_len >= chips_to_cover[c][0])
    M.c3b = pyomo.Constraint(M.F, M.C, rule=c3b_rule)

    def c3c_rule(M, f, c):
        return (M.Fklly[f] - (1 - M.X1[f,c])*chip_y_len <= chips_to_cover[c][1]) 
    M.c3c = pyomo.Constraint(M.F, M.C, rule=c3c_rule)

    def c3d_rule(M, f, c):
        return (M.Fkury[f] + (1 - M.X1[f,c])*chip_y_len >= chips_to_cover[c][1]) 
    M.c3d = pyomo.Constraint(M.F, M.C, rule=c3d_rule)

    def c3j_rule(M, c):
        return sum(M.X1[f,c] for f in M.F) == 1
    M.c3j = pyomo.Constraint(M.C, rule=c3j_rule)

    #
    # Define the sub-footpring area through two Pyomo.Constraints 
    #
